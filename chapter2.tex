...
\emph{Base case} ($n=1$): $A=[(A)_{11}]$, so $\det(A)=(A)_{11}$.

\emph{Inductive step:} let $A$ be $n\times n$ upper-triangular. Its first column is $((A)_{11},0,\dots,0)\trans$, so cofactor expansion down column $1$ leaves only the first term,
\[
...
\begin{prf}
Each elementary matrix $E$ is obtained from $I_n$ by its named row operation, and $\det(I_n)=1$ by \Lem{2.2.1}. Applying the effect of that operation on determinants (\Thm{2.2.3}) to $I_n$:

\case{(a)} Scaling a row by $k$ multiplies the determinant by $k$, so $\det(E)=k\det(I_n)=k$.

\case{(b)} Interchanging two rows negates the determinant, so $\det(E)=-\det(I_n)=-1$.

\case{(c)} Adding a multiple of one row to another leaves the determinant unchanged, so $\det(E)=\det(I_n)=1$.
\end{prf}
...
\begin{prf}
By \Thm{1.5.4}, $EA$ is the matrix obtained by applying $E$'s row operation to $A$. There are three types of elementary matrix, one per row operation; we check each, using \Thm{2.2.3} for the effect on $\det(EA)$ and \Thm{2.2.6} for $\det(E)$.

\case{Scaling} If $E$ scales row $i$ by $k$, then $\det(EA)=k\det(A)$ and $\det(E)=k$, so $\det(EA)=k\det(A)=\det(E)\det(A)$.

\case{Interchange} If $E$ interchanges two rows, then $\det(EA)=-\det(A)$ and $\det(E)=-1$, so $\det(EA)=-\det(A)=\det(E)\det(A)$.

\case{Row addition} If $E$ adds a multiple of one row to another, then $\det(EA)=\det(A)$ and $\det(E)=1$, so $\det(EA)=\det(A)=\det(E)\det(A)$.

\noindent In every case $\det(EA)=\det(E)\det(A)$.
\end{prf}
