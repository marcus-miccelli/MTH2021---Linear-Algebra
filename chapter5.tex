...
\begin{prf}
If $\vu=\vze$ both sides are $0$. Otherwise, for any $t\in\R$ consider
\[
0\le\inner{t\vu+\vv}{t\vu+\vv}=\norm{\vu}^2t^2+2\inner{\vu}{\vv}t+\norm{\vv}^2,
\]
using \Lem{5.1.3} and linearity. This quadratic in $t$ is nonnegative for all $t$, so its discriminant is $\le0$:
\[
\bigl(2\inner{\vu}{\vv}\bigr)^2-4\norm{\vu}^2\norm{\vv}^2\le0\;\Longrightarrow\;\inner{\vu}{\vv}^2\le\norm{\vu}^2\norm{\vv}^2.
\]
Taking square roots gives $\abs{\inner{\vu}{\vv}}\le\norm{\vu}\norm{\vv}$.
\end{prf}
...
\begin{prf}
$\vze\in U^\perp$ since $\inner{\vze}{\vu}=0$ for all $\vu$. If $\vx,\vy\in U^\perp$ and $k$ is a scalar, then for every $\vu\in U$, $\inner{\vx+k\vy}{\vu}=\inner{\vx}{\vu}+k\inner{\vy}{\vu}=0+0=0$ by \Lem{5.1.3}, so $\vx+k\vy\in U^\perp$. By the Subspace Test, $U^\perp$ is a subspace.
\end{prf}
...
\begin{prf}
Take an orthonormal basis $\{\vv_1,\dots,\vv_k\}$ of $W$ (which exists by \Thm{5.4.10} below) and set $\vw=\sum_{j=1}^k\inner{\vu}{\vv_j}\vv_j\in W$ and $\vw^\perp=\vu-\vw$. For each $i$, $\inner{\vw^\perp}{\vv_i}=\inner{\vu}{\vv_i}-\sum_j\inner{\vu}{\vv_j}\inner{\vv_j}{\vv_i}=\inner{\vu}{\vv_i}-\inner{\vu}{\vv_i}=0$, so $\vw^\perp\perp W$. Uniqueness: if $\vu=\vw_1+\vw_1^\perp=\vw_2+\vw_2^\perp$, then $\vw_1-\vw_2=\vw_2^\perp-\vw_1^\perp\in W\cap W^\perp=\{\vze\}$ (\Lem{5.3.2}), so the decompositions agree.
\end{prf}
...
\begin{prf}
Start from any basis $\{\vu_1,\dots,\vu_n\}$ and apply the Gram--Schmidt process (\Alg{5.4.11}) to produce an orthogonal basis $\{\vv_1,\dots,\vv_n\}$; then normalise each $\vv_j$. The construction (verified in \Alg{5.4.11}) keeps each $\vv_j$ nonzero and the spans equal, so the result is an orthonormal basis.
\end{prf}
...
\begin{multicols}{2}
\begin{enumerate}[label=\textup{(\alph*)},leftmargin=1.6em,itemsep=0pt]
...
\begin{prf}
Conditions (a)--(g) are \Thm{2.2.12}. For the rest, $A$ is $n\times n$, so by the Fundamental Theorem the pivot count $r$ satisfies: (n) $\rank(A)=n\iff r=n\iff\RREF(A)=I_n$, which is (c); (o) $\nullity(A)=0\iff n-r=0\iff r=n$, again (n); (h),(j),(l),(n) all say the $n$ columns are independent / spanning / a basis / rank $n$, equivalent to $r=n$ by \Thm{3.3.13}; (i),(k),(m) are the row versions, equivalent via $\det(A)=\det(A\trans)$ (\Thm{2.2.2}); (p) $\ran(T_A)=\colsp(A)=\R^n\iff$ (j). All reduce to $r=n$, i.e. to (a).
\end{prf}
...
\begin{prf}
By \Thm{5.6.4}, $\vx$ is least-squares $\iff A\vx=\proj_W\vb$ ($W=\colsp(A)$) $\iff\vb-A\vx\in W^\perp$. Now $W^\perp=\colsp(A)^\perp=\ker(A\trans)$ (\Thm{5.5.1} applied to $A\trans$, since $\rowsp(A\trans)=\colsp(A)$). So the condition is $A\trans(\vb-A\vx)=\vze$, i.e. $A\trans A\vx=A\trans\vb$. Consistency: $\proj_W\vb\in W=\colsp(A)$, so some $\vx$ achieves $A\vx=\proj_W\vb$, and that $\vx$ solves the normal system.
\end{prf}
