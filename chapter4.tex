...
\begin{prf}
\case{(a)} $\ker(T)\neq\varnothing$ since $T(\vze)=\vze$ (\Lem{4.2.2}). If $\vu,\vv\in\ker(T)$ then $T(\vu+k\vv)=T(\vu)+kT(\vv)=\vze+k\vze=\vze$, so $\vu+k\vv\in\ker(T)$; closure under both operations follows (take $k=1$ for addition, $\vu=\vze$ for scalars). By the Subspace Test, $\ker(T)$ is a subspace.

\case{(b)} $\ran(T)\neq\varnothing$ since $\vze=T(\vze)\in\ran(T)$. If $\vy_1=T(\vx_1)$ and $\vy_2=T(\vx_2)$ lie in $\ran(T)$, then $\vy_1+k\vy_2=T(\vx_1)+kT(\vx_2)=T(\vx_1+k\vx_2)\in\ran(T)$. By the Subspace Test, $\ran(T)$ is a subspace.
\end{prf}
...
\begin{prf}
Let $k=\nullity(T)$. If $k=n$ then $\ker(T)=V$, so $T$ is the zero map, $\ran(T)=\{\vze\}$, $\rank(T)=0$, and the identity holds. Otherwise $0\le k<n$. Choose a basis $\{\vv_1,\dots,\vv_k\}$ of $\ker(T)$ and extend it to a basis $\{\vv_1,\dots,\vv_k,\vv_{k+1},\dots,\vv_n\}$ of $V$ (\Thm{3.3.15}). We claim $\{T(\vv_{k+1}),\dots,T(\vv_n)\}$ is a basis of $\ran(T)$; granting this, $\rank(T)=n-k$ and the identity follows.

\emph{Spanning.} Any $\vy\in\ran(T)$ is $\vy=T(\vx)$ for some $\vx=\sum_{i=1}^n c_i\vv_i$. Since $T(\vv_i)=\vze$ for $i\le k$,
\[
\vy=T(\vx)=\sum_{i=1}^n c_iT(\vv_i)=\sum_{i=k+1}^n c_iT(\vv_i),
\]
a combination of $T(\vv_{k+1}),\dots,T(\vv_n)$. \emph{Independence.} Suppose $\sum_{i=k+1}^n d_iT(\vv_i)=\vze$. Then $T\bigl(\sum_{i=k+1}^n d_i\vv_i\bigr)=\vze$, so $\sum_{i=k+1}^n d_i\vv_i\in\ker(T)$, and is therefore a combination $\sum_{i=1}^k e_i\vv_i$ of the kernel basis. This gives a relation $\sum_{i=1}^k e_i\vv_i-\sum_{i=k+1}^n d_i\vv_i=\vze$ among the basis $\{\vv_1,\dots,\vv_n\}$ of $V$, forcing all coefficients to vanish; in particular every $d_i=0$. Hence the images are independent.
\end{prf}
